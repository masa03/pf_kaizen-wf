...
3. 実装完了時チェックリスト（すべて完了してからarchive移動）:
   - [ ] proposal.md の内容を spec/ の各ファイルに分配マージ
   - [ ] backlog.md のステータスを【完了】に移動
   - [ ] changes/v2-xxx/ を changes/archive/ に移動
   - [ ] backlog.md のリンクを archive パスに更新
```
...
4. proposal-writer (pass=create): 回答を反映して proposal.md 作成
   ↓
5. proposal完成 → spec-reviewer (mode=proposal) で整合性レビュー ← **自動起動（必須）**
   ↓
6. レビュー指摘の修正 → 実装開始
   ↓
7. 成果物完成 → spec-reviewer (mode=deliverable) で成果物レビュー ← **自動起動（必須）**
   **重要**: proposal経由で実装した成果物（YAML/フローHTML/スクリプト等）を完成させたら、ユーザーへの提示前に必ず spec-reviewer (mode=deliverable) を起動すること。
   proposal経由でない修正・バグ修正・小変更は、ユーザーから「レビューして」と指示があった場合のみ起動する。
   **レビュー結果は必ずチャットでユーザーに提示すること。** 指摘がゼロの場合も「レビュー実施済み・指摘なし」と明示する。指摘があった場合は修正内容とあわせて報告すること。
   ↓
...
### spec-reviewer が確認すべき整合性（proposal レビュー時）

`spec-reviewer (mode=proposal)` は、proposal.md の**セクション間の内部整合性**を必ず確認すること:

- **フローチャート ↔ フロー設計**: フローチャートに記載されたステップが、フロー設計の各アクションに対応しているか。チャートに「通知」と書いてあればフロー設計にも通知アクションが存在するか
- **リスト設計 ↔ フロー設計**: フローで参照している列名がリスト設計に存在するか
- **画面設計 ↔ フロー設計**: 画面から呼び出すフローのパラメータが、フロー設計のトリガー定義と一致するか

設計矛盾（チャートに記載があるがフロー設計に手段がない等）を発見した場合は、**実装前に必ず指摘**すること。
...
| ファイル | 内容 | 読むタイミング |
|---|---|---|
| `knowledge/design-guidelines.md` | **このプロジェクトの設計方針**（変数スコープ・命名規則・リスト設計・分岐方針等） | **設計前・実装前（必須）** |
| `knowledge/powerapps.md` | YAML記法、コントロール、Code View制限、SP列名 | Power Apps画面タスク着手時 |
| `knowledge/automate.md` | 過去トラブル・ハマりどころ（失敗対策） | Power Automateフロー関連タスク着手時 |

**ルール**:
- `knowledge/design-guidelines.md` は**設計・実装を始める前に必ず読むこと**。このプロジェクトの設計判断の拠り所であり、方針から外れた実装をしてはならない。
- 他の知見ファイルも該当タスクに着手する前に読むこと。知見を読まずに実装を開始してはならない。
- 新たな設計判断（方針の選択・トレードオフの決定）をした際は、`knowledge/design-guidelines.md` に追記すること。
...
### フロー手順書を新規作成する前に必ず行うこと

新しいフロー手順書（`flow-*-build.html`）を作成する前に、**既存の手順書ファイル一覧を確認**し、設計方針・スタイルを揃えること。

```
// 確認コマンド例
ls powerautomate/flow-*-build.html
```

既存ファイルの構造・変数設計・アクション参照パターンと一貫性を保つ。特にスタイル（CSS）・コピーボタン実装・式のフォーマットは既存ファイルから流用すること。
