...
7. 成果物完成 → spec-reviewer (mode=deliverable) で成果物レビュー ← **自動起動（必須）**
   **重要**: 成果物（YAML/フローHTML/スクリプト等）を完成させたら、ユーザーへの提示前に必ず spec-reviewer (mode=deliverable) を起動すること。
   起動をスキップする場合は、理由をユーザーに明示して許可を得ること。「スキップしてよい」という判断を Claude 自身が行ってはならない。
   ↓
