...
**2026-04-01 調査・検証により判明した制約。クライアントと仕様を決めてから再実装。**

### 現状の制約まとめ

新方式（EditForm + ステージングリスト）採用により以下の制約が確定している。

| 機能 | 状況 | 理由 |
|---|---|---|
| プレビュー時の画像表示 | **不可** | EditFormはBase64を経由しないためContentBase64が取得できない |
| PDFのインライン表示 | **不可** | PDF Viewerコントロールは実験的ステータス + SP認証ファイル非対応 |
| 閲覧画面での画像インライン表示 | **可** | 提出後はSPドキュメントライブラリのURLでImage表示可能 |
| 閲覧画面でのPDF表示 | **リンクのみ** | クリックでSP/ブラウザPDFビューアが開く |

### 確認すべき仕様ポイント

#### Q1: FileCategoryそのものをなくすか？

PDFがインライン表示できない・プレビューもできない状況では、改善前/改善後/その他のカテゴリ分類のメリットが大幅に減少する。

**カテゴリを維持するメリット**
- 閲覧・評価画面で「改善前」「改善後」の画像を左右2カラムでインライン表示できる（画像ファイルの場合のみ）
- 評価者が申請内容を視覚的に把握しやすい

**カテゴリをなくすメリット**
- UI・実装がシンプルになる（ドロップダウン廃止、ステージングレコード1本化）
- ユーザーの操作ステップが減る
- Power Automateフローのカテゴリ別ループが不要になる

**判断の分岐**
- 改善前・改善後を画像（JPG/PNG/GIF）に限定してインライン表示を維持するなら → カテゴリを残してAddMediaButton方式に戻す
- 画像もPDFも全て「リンク表示のみ」で統一するなら → カテゴリを廃止してシンプルな実装に変更

#### Q2: 添付ファイルに画像のインライン表示は必要か？

評価者・承認者が改善前後の画像をアプリ内で確認できることは業務上どの程度重要か。

- 重要度が高い → カテゴリ維持 + 画像はAddMediaButton方式に戻す（PDF等は別途リンク）
- なくても問題ない → カテゴリ廃止 + 全ファイルをリンク表示のみに統一

#### Q3: プレビュー機能の添付ファイル表示は必要か？

提出前のプレビューで添付ファイルを確認する必要があるか。現状は「（添付済み）」のテキスト表示のみ。

---

### PDF Viewer調査結果（参考）

- Power AppsにはPDF Viewerコントロール（実験的）が存在するが、2022年から実験的ステータスが変わっていない
- SharePointの認証付きファイルは直接表示できない制限がある
- Power Automate経由でbase64取得する回避策は技術的に可能だが、フロー実行のラグ（数秒）・15,000人規模でのフロー実行数制約・実験的コントロールの安定性リスクがある
- **結論: 本番システムへの採用は推奨しない。リンク表示で対応するのが現実的。**
