...
### 改善WF_ステージングクリーンアップ（新規）

**トリガー**: スケジュール（繰り返し: 毎日1回）

**目的**: 提出・下書き保存されずに放置されたステージングレコードを自動削除する。フォームを開いた時点でステージングレコードが作成されるため、ブラウザを閉じて離脱した場合にゴミレコード（空レコード・ファイル付き）がSPリストに残り続ける問題を防ぐ。

**処理フロー:**

1. 「添付ファイルステージング」リストから、作成日時が48時間以上前のレコードを取得（ODataフィルタ: `Created lt '@{addHours(utcNow(), -48)}'`）
2. 取得したレコードを Apply to each でループ
3. 各レコードを削除（Delete item）

**設計判断:**
- 48時間の猶予により、長時間の入力作業中にレコードが消えるリスクを回避
- 下書き保存時にステージング転送が実行されるため、正常に保存されたファイルには影響しない
- 空レコード・ファイル付きレコードを区別せず一律削除（48時間経過で未使用と判断）
...
| `docs/spec/flows.md` フロー一覧 | 「改善WF_添付ファイルアップロード」を廃止として注記（フロー自体は既存申請データ対応のため削除しない）。「改善WF_ステージングクリーンアップ」を追加 |
...
3. 新規Power Automateフロー `改善WF_ステージングクリーンアップ` を作成（スケジュールトリガー、毎日1回）
4. `editFormAttachment` (EditForm) を配置・設定
