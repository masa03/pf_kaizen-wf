...
### §12 申請完了・承認完了後のサンクス画面

申請完了時・承認完了時のリダイレクト先を現在の「タブを閉じる（Exit()）」からサンクス画面に変更する。
...
**実装方針（未確定）:**
- サンクス画面（ThankYouScreen）を新規作成
- 表示メッセージは変数（`varThankYouMessage`等）で切り替え
- 閉じるボタンは不要（メッセージ表示のみ）
- 変更ファイル: `screen-evaluation.yaml`（承認・差戻完了後の遷移）、`screen-application-form.yaml`（申請完了後の遷移）、`app-startscreen.pfx`（必要に応じて）
...
- **§1 添付資料の多ファイル形式対応・容量表記** — [proposal](changes/v2-file-format/proposal.md)（仕様確定 4/12、実装未着手）
