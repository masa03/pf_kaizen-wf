...
### §13 承認リストのカスタムViewにそのまま承認へ遷移するリンクを付与

SharePointの「自分の承認リスト」カスタムビューで、各行のリンク先URLに承認画面へ直接遷移するためのパラメータを付与する。

**要件:**
- 自分の申請リスト: 現状のまま（申請詳細・閲覧画面へのリンク）
- 自分の承認リスト: 承認画面（EvaluationScreen）へ直接遷移できるURLパラメータ付与（例: `?RequestID=xxx&EvalType=課長` 相当）

**実装方針（未確定）:**
- SharePoint Column Formatting（JSON）でリンクURLを動的生成（SharePointのビューのリンク列を書き換える）
- `EvalType` はステータス列（`課長評価中` → `課長`、`部長評価中` → `部長`）から判定
- 変更ファイル: `scripts/set-column-formatting.ps1`
