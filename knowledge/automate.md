...
**重要**: Power AutomateのUI上で必須表示される列は、SharePointリストの`Required`設定と一致しない。テキスト型（1行・複数行）・日付型の必須列のみUI上で入力必須となり、**ユーザー型・選択肢型・数値型の列はリスト定義でRequired=TRUEでもUI上では必須表示されない**。
